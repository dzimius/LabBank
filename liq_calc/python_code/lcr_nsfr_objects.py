...
# ---------------------------------------------------------------------------
# IRRBB report builder (NII + EVE only, with worst-scenario rows)
# ---------------------------------------------------------------------------

def build_irrbb_report(
    report_date: pd.Timestamp,
    nii_by_scenario: pd.DataFrame,
    eve_by_scenario: pd.DataFrame,
    tier1_capital: float = 1.0,
) -> pd.DataFrame:
    """Assemble the IRRBB report table (NII + EVE only).

    Parameters
    ----------
    report_date      : valuation date
    nii_by_scenario  : columns: currency, scenario_id, nii_total
    eve_by_scenario  : columns: currency, scenario_id, pv_total
    tier1_capital    : Tier 1 capital used for SOT ratios (same currency units as NII/EVE)

    Returns
    -------
    DataFrame with columns:
      report_date, currency, scenario_id,
      nii_base, nii_shocked, delta_nii, sot_nii_pct,
      eve_base, eve_shocked, delta_eve, sot_eve_pct

    sot_nii_pct = delta_nii / tier1_capital * 100  (EBA threshold: must not be < -5%)
    sot_eve_pct = delta_eve / tier1_capital * 100  (EBA threshold: must not be < -15%)

    Two extra rows per currency are appended at the end:
      scenario_id='worst_nii' — scenario with minimum delta_nii
      scenario_id='worst_eve' — scenario with minimum delta_eve
    """
    # Base NII / EVE per currency
    nii_base = (
        nii_by_scenario[nii_by_scenario["scenario_id"] == "base"]
        .set_index("currency")["nii_total"]
    )
    eve_base = (
        eve_by_scenario[eve_by_scenario["scenario_id"] == "base"]
        .set_index("currency")["pv_total"]
    )

    all_scenarios = nii_by_scenario["scenario_id"].unique().tolist()
    # Ensure base is first, exclude 'base' from shocked scenarios
    shocked_scenarios = [s for s in all_scenarios if s != "base"]
    ordered_scenarios = (["base"] + shocked_scenarios) if "base" in all_scenarios else all_scenarios

    currencies = nii_by_scenario["currency"].unique().tolist()

    rows = []
    for ccy in currencies:
        nii_b = float(nii_base.get(ccy, 0.0))
        eve_b = float(eve_base.get(ccy, 0.0))

        for scen in ordered_scenarios:
            nii_s = float(nii_by_scenario[
                (nii_by_scenario["currency"] == ccy) &
                (nii_by_scenario["scenario_id"] == scen)
            ]["nii_total"].sum())
            eve_s = float(eve_by_scenario[
                (eve_by_scenario["currency"] == ccy) &
                (eve_by_scenario["scenario_id"] == scen)
            ]["pv_total"].sum())
            ...
            rows.append({
                ...
                "nii_shocked":  nii_s,
                "delta_nii":    d_nii,
                "sot_nii_pct":  d_nii / tier1_capital * 100.0 if tier1_capital else None,
                "eve_base":     eve_b,
                "eve_shocked":  eve_s,
                "delta_eve":    d_eve,
                "sot_eve_pct":  d_eve / tier1_capital * 100.0 if tier1_capital else None,
            })

    df = pd.DataFrame(rows)

    # Add worst_nii and worst_eve rows per currency (exclude base from search)
    worst_rows = []
    shocked_df = df[df["scenario_id"] != "base"]

    for ccy in currencies:
        ccy_shocked = shocked_df[shocked_df["currency"] == ccy]
        if ccy_shocked.empty:
            continue

        # worst_nii: scenario with minimum delta_nii
        worst_nii_row = ccy_shocked.loc[ccy_shocked["delta_nii"].idxmin()].copy()
        worst_nii_row["scenario_id"] = "worst_nii"
        worst_rows.append(worst_nii_row)

        # worst_eve: scenario with minimum delta_eve
        worst_eve_row = ccy_shocked.loc[ccy_shocked["delta_eve"].idxmin()].copy()
        worst_eve_row["scenario_id"] = "worst_eve"
        worst_rows.append(worst_eve_row)

    if worst_rows:
        df = pd.concat([df, pd.DataFrame(worst_rows)], ignore_index=True)
