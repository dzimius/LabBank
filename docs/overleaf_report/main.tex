...
how the master ledger is \textit{structured}. \texttt{dbo.transactions} is the hub
--- the fact table every regulatory/finance report in this project ultimately reads
from, one row per position. Each \texttt{schemat.*} table adds product-specific
detail for that same \texttt{transaction\_id} --- a 1:1 relationship, since a given
position is a loan, a deposit, a security, or an equity item, never more than one
at once.
